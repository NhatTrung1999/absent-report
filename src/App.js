...
const validationSchema = Yup.object({
  ...
  DependentID: Yup.string().required("Vui lòng không để trống!"),
  DependentName: Yup.string().required("Số thẻ không tồn tại!"),
  Reason: Yup.string().trim().required("Vui lòng nhập lý do xin nghỉ!"),
});
...
function App() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, action) => {
      ...
      axios
        .post(
          "http://www.lacty.com.vn/Report_Vacation/data/data_report_vacation.php?action=addReportVacation",
  ...
  const handleKeyUp = () => {
    ...
    const payload = {
      DependentID: formik.values.DependentID,
    };

    axios
      .post(
        "http://www.lacty.com.vn/Report_Vacation/data/data_report_vacation.php?action=getName",
        JSON.stringify(payload)
      )
      .then((res) => {
        setGetName(res.data);
  ...
  formik.values.DependentName = getName;
  ...
  return (
    <>
      ...
      <Container>
              ...
              <Grid item xs={12} md={6}>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <TextField
                      fullWidth
                      label="Số thẻ người thay thế"
                      inputProps={{ maxLength: 5 }}
                      name="DependentID"
                      value={formik.values.DependentID}
                      onChange={formik.handleChange}
                      onKeyUp={handleKeyUp}
                      error={
                        formik.touched.DependentID &&
                        Boolean(formik.errors.DependentID)
                      }
                      helperText={
                        formik.touched.DependentID && formik.errors.DependentID
                      }
                    />
                  </Grid>
                  <Grid item xs={12} md={6}>
        ...
        <Card variant="outlined" sx={{ marginBottom: 3 }}>
          <ListRegister open={open} showId={formik.values.ID} />
        </Card>
        ...
        {open === false ? (
          ...
          >
            ...
            >
              ...
              Bạn đã đăng kí nghỉ phép thất bại!
            </Alert>
          </Snackbar>
        ) : (
          <Box></Box>
